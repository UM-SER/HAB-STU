...
df_ave_allcell <- read_dta("/data/algae_reg_allcell.dta") 
df_ave_allcell$GEOID <- format(df_ave_allcell$GEOID, scientific = FALSE)

df_allCT <- read_dta("/data/algae_reg_allCT.dta") 
df_allCT$GEOID <- format(df_allCT$GEOID, scientific = FALSE)
...
##############################################################################
###          Figure S6 Effects of HAB using the restricted sample          ###
##############################################################################
GEOID_to_remove <- df %>%
  group_by(GEOID) %>%
...
figureS6d <- wrap_elements(g2)    
figureS6d

figureS6_withoutlegend <- (figureS6a | figureS6b) /
  (figureS6c | figureS6d) +
  plot_layout(
    widths  = c(1, 1),   
    heights = c(1, 1)   
...
########################################################################################################
###     Figure S7 Effects of HAB using average concentration based on all monitoring observations    ###
########################################################################################################
...
p_top <- ggplot(coef_df, aes(x = model_f, y = estimate)) +
  ...
  scale_y_continuous(
    breaks = c(-0.0015, -0.0010, -0.0005, 0),
...
figureS7d <- wrap_elements(g2)    
figureS7d

figureS7_withoutlegend <- (figureS7a | figureS7b) /
  (figureS7c | figureS7d) +
  plot_layout(
    widths  = c(1, 1),   
    heights = c(1, 1)   
...
figureS7 <- figureS7_withoutlegend / p_legend +
  plot_layout(heights = c(6,6, 0.5))   

figureS7

###################################################################################
###       Figure S8 Effects of HAB using using the full Florida sample          ###
###################################################################################
...
figureS8d <- wrap_elements(g2)    
figureS8d

figureS8_withoutlegend <- (figureS8a | figureS8b) /
  (figureS8c | figureS8d) +
  plot_layout(
    widths  = c(1, 1),   
    heights = c(1, 1)   
...
figureS8 <- figureS8_withoutlegend / p_legend +
  plot_layout(heights = c(6,6, 0.5))   
...
##############################################################################
###             Figure S9 Effects of HAB using a 1-mile buffer             ###
##############################################################################
...
figureS9d <- wrap_elements(g2)    
figureS9d

figureS9_withoutlegend <- (figureS9a | figureS9b) /
  (figureS9c | figureS9d) +
  plot_layout(
    widths  = c(1, 1),   
    heights = c(1, 1)   
...
figureS9 <- figureS9_withoutlegend / p_legend +
  plot_layout(heights = c(6,6, 0.5))   

figureS9

##############################################################################
###             Figure S10 Effects of HAB using a 3-mile buffer            ###
##############################################################################
...
figureS10d <- wrap_elements(g2)    
figureS10d

figureS10_withoutlegend <- (figureS10a | figureS10b) /
  (figureS10c | figureS10d) +
  plot_layout(
    widths  = c(1, 1),   
    heights = c(1, 1)   
  )

figureS10_withoutlegend
...
figureS10 <- figureS10_withoutlegend / p_legend +
  plot_layout(heights = c(6,6, 0.5))   

figureS10

##############################################################################
###   Figure S11 Effects of HAB using the sample excluding the year 2020   ###
##############################################################################
df_e2020 <- df %>%
  filter(year != 2020)
...
figureS11d <- wrap_elements(g2)    
figureS11d

figureS11_withoutlegend <- (figureS11a | figureS11b) /
  (figureS11c | figureS11d) +
  plot_layout(
    widths  = c(1, 1),   
    heights = c(1, 1)   
  )

figureS11_withoutlegend
...
figureS11 <- figureS11_withoutlegend / p_legend +
  plot_layout(heights = c(6,6, 0.5))   

figureS11

##############################################################################
###              Figure S12 Dynamic effects of HAB on time use             ###
##############################################################################
df <- df %>%
...
figureS12a <- ggplot(coef_time_lag_lead, aes(x = var_name, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat), size = 4, color = "#C84747") +
...
figureS12c <- ggplot(coef_visit_lag_lead, aes(x = var_name, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat), size = 4, color = "#C84747") +
...
figureS12b <- ggplot(coef_time_total_lag_lead, aes(x = var_name, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat), size = 4, color = "#C84747") +
...
figureS12d <- ggplot(coef_visit_total_lag_lead, aes(x = var_name, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat), size = 4, color = "#C84747") +
...
figureS12_1 <- figureS12a + plot_spacer() + figureS12b +
  plot_layout(widths = c(1, 0.03, 1))   

figureS12_2 <- figureS12c + plot_spacer() + figureS12d +
  plot_layout(widths = c(1, 0.03, 1))   

figureS12_withoutlegend <- figureS12_1/figureS12_2
...
figureS12 <- figureS12_withoutlegend / p_legend +
  plot_layout(heights = c(5, 5, 0.5))   

figureS12

######################################################################################
###   Figure S13 Heterogeneous effects of average HAB by socio-demographic group   ###
######################################################################################
df_socio <- df %>%
  left_join(
    socio %>%
...
figureS13a <- ggplot(coef_df, aes(x = group, y = estimate)) +
...
figureS13e <- ggplot(coef_df, aes(x = group, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat),
...
figureS13b <- ggplot(coef_df, aes(x = group, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat),
...
figureS13f <- ggplot(coef_df, aes(x = group, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat),
...
figureS13c <- ggplot(coef_df, aes(x = group, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat),
...
figureS13g <- ggplot(coef_df, aes(x = group, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat),
...
figureS13d <- ggplot(coef_df, aes(x = group, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat),
...
figureS13h <- ggplot(coef_df, aes(x = group, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat),
...
figureS13_withoutlegend <- (figureS13a | figureS13b | figureS13c | figureS13d) /
  (figureS13e | figureS13f | figureS13g | figureS13h)
figureS13_withoutlegend
...
figureS13 <- figureS13_withoutlegend / p_legend +
  plot_layout(heights = c(5, 5, 0.5))   

figureS13

#####################################################################################
###    Figure S14 Heterogeneous effects of total HAB by socio-demographic group   ###
#####################################################################################
...
figureS14a <- ggplot(coef_df, aes(x = group, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat),
...
figureS14e <- ggplot(coef_df, aes(x = group, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat),
...
figureS14b <- ggplot(coef_df, aes(x = group, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat),
...
figureS14f <- ggplot(coef_df, aes(x = group, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat),
...
figureS14c <- ggplot(coef_df, aes(x = group, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat),
...
figureS14g <- ggplot(coef_df, aes(x = group, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat),
...
figureS14d <- ggplot(coef_df, aes(x = group, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat),
...
figureS14h <- ggplot(coef_df, aes(x = group, y = estimate)) +
  geom_linerange(aes(ymin = lower, ymax = upper),
                 linewidth = 0.6, color = "#C84747") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(aes(shape = sig_cat),
...
figureS14_withoutlegend <- (figureS14a | figureS14b | figureS14c | figureS14d) /
  (figureS14e | figureS14f | figureS14g | figureS14h)
...
figureS14 <- figureS14_withoutlegend / p_legend +
  plot_layout(heights = c(5, 5, 0.5))   
